...
void Graphics:: drawMenuTex(SDL_Surface* menu,SDL_Rect &p)
{
  SDL_Texture *texture = SDL_CreateTextureFromSurface(mRenderer, menu);
  SDL_RenderCopy(mRenderer, texture, NULL, &p);
  SDL_DestroyTexture(texture);
}//
void Graphics::RenderRect( SDL_Rect mRenderRect,SDL_Color color)
{

    //SDL_Color color = {255, 255, 255};
    SDL_SetRenderDrawColor(mRenderer, color.r, color.g, color.b, 255);
    SDL_RenderFillRect(mRenderer, &mRenderRect);
